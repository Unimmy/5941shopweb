			...
			if(data.status!=200){alert(data.message)}
				...
				$.each(message, function(index, item) {
					...
					str+="<td>"+(index+1)+"</td>";
						...
						if(item.bh==""||item.bh==null||item.bh==undefined){str+="<td></td>";}else{str+="<td>"+item.bh+"</td>";}
						if(item.type==""||item.type==null||item.type==undefined){str+="<td></td>";}else{
							if(item.type==1){str+="<td>订单数量</td>";}else if(item.type==2){str+="<td>人数</td>";}else{str+="<td>商品数量</td>";}
						}
